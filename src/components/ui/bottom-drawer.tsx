...
export function BottomDrawer({
  ...
  const handleDragEnd = (_: any, info: PanInfo) => {
    ...
    const shouldSnapToMax = isMovingUp && (currentPosition > initialHeightPx + snapThresholdPx || Math.abs(dragDistance) > snapThresholdPx);
    if (shouldSnapToMax) {
      controls.start({
  ...
  return <AnimatePresence>
      ...
      height: `${maxHeight}vh`
          ...
          {/* Drag Handle */}
          <div className="absolute top-0 left-0 right-0 h-8 flex items-center justify-center cursor-grab active:cursor-grabbing touch-none bg-gray-900 hover:bg-gray-800">
